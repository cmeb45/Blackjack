public class GameEngine{
	...
	NumberPlayers = IO.readInt();
        ...
        // Creates a new Player array, such that each element of the Player array represents a distinct player
		...
		System.out.println("Hint 2: Follow the Basic Blackjack Strategy, but don't be afraid to follow you intuition, no matter what");
        ...
        for(int i = 1; i <= NumberPlayers; i++){
			...
			System.out.println(PlayerArray[i].Name+", you have "+PlayerArray[i].Bankroll+" in your bankroll. How much would you like to wager?");
	    ...
	    if(PlayerArray[0].Hand[1].getValue() == 1){
			...
			for(int i = 1; i <= NumberPlayers; i++){
			System.out.println("The dealer has shown an Ace on his/her first card. "+PlayerArray[i].Name+", would you like to make an insurance bet? Enter 0 for 'No' and 1 for 'Yes'");
			int z;
			z = IO.readInt();
            while(z != 0 && z != 1){
			...
			if(PlayerArray[i].getInsurance == 1){
				...
				// If the player decides to get insurance, make a side bet that is half of the original wager
			}
			}
	    }
        else{
		...
		for(int i = 1; i <= NumberPlayers; i++){
		  if(PlayerArray[i].Hand[0].getFace() == PlayerArray[i].Hand[1].getFace()){
		    System.out.println(PlayerArray[i].Name+", you have a pair of "+PlayerArray[i].Hand[0].getFace()+"'s. Would you like to split? Enter 1 for Yes and 0 for No");
			System.out.println("According to Basic Blackjack Strategy, "+BasicBlackjackStrategy.BeforeSplitStrategyHints(PlayerArray[i], PlayerArray[0]));
		    int q;
			q = IO.readInt();
			while(q != 0 && q != 1){
			// If the user enters a number that he/she wasn't supposed to enter
        ...
        for(int i = 1; i <= NumberPlayers; i++){
	       ...
	       while(PlayerArray[i].getBusted == 0 && PlayerArray[i].Stand == 0 && DeckCounter < 52 && PlayerArray[i].Split == 0){
		   // While the player has not busted, split, or requested a stand, and while the number of cards passed out is less than or equal to the number of cards in the main deck
				for(int j = 2; j < 11; j++){
                System.out.println(PlayerArray[i].Name+"'s current score is "+PlayerArray[i].Score);
				PlayerArray[i].CardCounter = HiLoCardCount.CardCounter(PlayerArray);
				System.out.println(PlayerArray[i].Name+"'s card counter is "+PlayerArray[i].CardCounter);
				// Updates the card counter with each hit
				System.out.println(BasicBlackjackStrategy.StrategyHints(PlayerArray[i], PlayerArray[0]));
				...
				// Gives the probability of busting if the player decides to hit
		          ...
		          // If the player chooses to hit
                    ...
                    if(PlayerArray[i].Hand[j].getValue() == 1 && PlayerArray[i].Score >= 11){
		            // If the player gets an ace and his/her score is at least 11
                        PlayerArray[i].Score = PlayerArray[i].Score + 1;
                        // Keep aces low, so that the player will not "bust" right away
                    }
                    else if(PlayerArray[i].Hand[j].getValue() == 1 && PlayerArray[i].Score < 11){
			        // If the player gets an ace and his/her score is less than 11
                        PlayerArray[i].Score = PlayerArray[i].Score + 11;
			        // Make aces high, so the player has a score that is closer to 21
		            }
                    else if(PlayerArray[i].Hand[j].getValue() != 1){
                        PlayerArray[i].Score = PlayerArray[i].Score + PlayerArray[i].Hand[j].getValue();
                    }
					PlayerArray[i].NumberOfCards++;
                    DeckCounter++;
		         }
	...
	if(PlayerArray[0].Hand[0].getValue() == 1 && PlayerArray[0].Score >= 11){
	    // The dealer turns his face-down card over
		    ...
		    for(int i = 1; i <= NumberPlayers; i++){
		       if(PlayerArray[i].getInsurance != 0){
		         System.out.println(PlayerArray[i].Name+" has lost the Insurance bet, and now has lost "+PlayerArray[i].Insurance);
		...
		if(PlayerArray[0].Hand[1].getValue() == 1 && PlayerArray[0].Hand[0].getValue() == 10 && PlayerArray[0].Score == 21){
		...
		for(int i = 1; i <= NumberPlayers; i++){
		  if(PlayerArray[i].getInsurance != 0){
		     System.out.println(PlayerArray[i].Name+" has won the Insurance bet, and now has an additional "+(2*PlayerArray[i].Insurance));
			 PlayerArray[i].Bankroll += (2*PlayerArray[i].Insurance);
		  }
		}
		}
		else if(PlayerArray[0].Hand[1].getValue() == 1 && PlayerArray[0].Hand[0].getValue() != 10){
		  System.out.println("The dealer has not turned up a natural 21");
		  for(int i = 1; i <= NumberPlayers; i++){
		       if(PlayerArray[i].getInsurance != 0){
		         System.out.println(PlayerArray[i].Name+" has lost the Insurance bet, and now has lost "+PlayerArray[i].Insurance);
    ...
    while(PlayerArray[0].Score < 17 && DeckCounter < 52 && Dealer_Card < 12){
	    // While the score of the dealer is less than 17, and while the number of cards passed out is less than or equal to the number of cards in the main deck
		...
		// Make aces high, so the player has a score that is closer to 21
               ...
               PlayerArray[0].Score = PlayerArray[0].Score + PlayerArray[0].Hand[Dealer_Card].getValue();
        }
        DeckCounter++;
	    Dealer_Card++;
	}
    if(PlayerArray[0].Score > 21){
        ...
        for(int i = 1; i <= NumberPlayers; i++){
		  if(PlayerArray[i].Split == 0){
		    if(PlayerArray[i].getBusted == 0){
			   ...
			   System.out.println(PlayerArray[i].Name+" has a blackjack, so he/she has won an additional "+(PlayerArray[i].Wager*1.5));
		   ...
		   for(int k = 0; k <= 1; k++){
		    if(PlayerArray[i].SplitBusted[k] == 0){
		       System.out.println(PlayerArray[i].Name+" has won on his/her Split Hand "+k+" with a score of "+PlayerArray[i].SplitScore[k]);
			   if(PlayerArray[i].Score != 21){
			   // If the player does not have a blackjack
			   System.out.println(PlayerArray[i].Name+" has won an additional "+PlayerArray[i].SplitWager[k]);
			   PlayerArray[i].Bankroll += PlayerArray[i].SplitWager[k];
			   // The player wins 1:1
			   }
			   else{
			   // If the player has a blackjack
			   System.out.println(PlayerArray[i].Name+" has a blackjack, so he/she has won an additional "+(PlayerArray[i].SplitWager[k]*1.5));
			   PlayerArray[i].Bankroll += (PlayerArray[i].SplitWager[k]*1.5);
			   }
		    }
            else if(PlayerArray[i].SplitBusted[k] == 1){
		       System.out.println(PlayerArray[i].Name+" has lost on his/her Split Hand "+k+" with a score of "+PlayerArray[i].Score);
		    }		   
		   }
		  }
	    }
        ...
        for(int i = 1; i <= NumberPlayers; i++){
		  if(PlayerArray[i].Split == 0){
            ...
            else if(PlayerArray[i].SplitScore[k] == PlayerArray[0].Score && PlayerArray[i].SplitBusted[k] == 0){
		    // If the player has an equal score to that of the dealer
		       System.out.println(PlayerArray[i].Name+" has pushed on his/her Split Hand "+k+" with a score of "+PlayerArray[i].SplitScore[k]);
		    }
		    else if(PlayerArray[i].SplitScore[k] < PlayerArray[0].Score && PlayerArray[i].SplitBusted[k] == 0){
		    // If the player has a lesser score than the dealer
		       System.out.println(PlayerArray[i].Name+" has lost on his/her Split Hand "+k+" with a score of "+PlayerArray[i].SplitScore[k]);
		    }
			else if(PlayerArray[i].SplitBusted[k] == 1){
			// If the player has busted, he/she automatically loses
			   System.out.println(PlayerArray[i].Name+" has lost on his/her Split Hand "+k+" with a score of "+PlayerArray[i].SplitScore[k]);
        ...
        System.out.println("That was fun! Would you like to play again? Enter 'Y' for 'Yes' and anything else for 'No'");
        Play = IO.readChar();
        if(Play != 'Y'){
	    System.out.println("Goodbye!");
            return;
