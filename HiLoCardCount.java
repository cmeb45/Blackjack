			...
			if(AllGamers[i].Split == 0){
				...
				for(int k = 0; k <= 1; k++){
					for(int j = 0; j < AllGamers[i].SplitNumberOfCards[k]; j++){
						if(AllGamers[i].SplitHand[k][j].getValue() >= 2 
							&& AllGamers[i].SplitHand[k][j].getValue() <=6){
					// If a 2, 3, 4, 5, or 6 is passed out
							Count++;
						}
						else if(AllGamers[i].SplitHand[k][j].getValue() >= 7 
							&& AllGamers[i].SplitHand[k][j].getValue() <= 9){
					// If a 7, 8, or 9 is passed out
							Count = Count + 0;
						}
						else if(AllGamers[i].SplitHand[k][j].getValue() == 10 
							|| AllGamers[i].SplitHand[k][j].getValue() == 1){
							// If a 10, J, Q, K, or A is passed out
							Count = Count - 1;
						}
					}
